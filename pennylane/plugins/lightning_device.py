...
class LightningDevice(QubitDevice):
    ...
    def mat_vec_product(self, mat, vec, wires):
        ...
        """
        # axis1 = list(range(len(wires), 2 * len(wires)))
        # axis2 = wires.tolist()
        ...
        operator = []  
        need_to_place_mat = True
        for idx in range(self.num_wires):
            if idx in wires:
                if need_to_place_mat:
                    operator.append(mat)
                    need_to_place_mat = False
            else:
                operator.append(identity)

        mat = functools.reduce(np.kron, operator)

        # TODO: Need to check dimension matching?
        dim = len(mat)
        a = lightning.matVecProduct(mat, vec, dim, dim)
        return a
